...
DROP TABLE IF EXISTS `advocate_case_assn`;
...
CREATE TABLE `advocate_case_assn`
(
...
ALTER TABLE  `login` ADD INDEX  `username_idxfk` (  `username` );
...
CREATE INDEX `case_id_idx` ON `case_suspect_assn`(`case_id`);
ALTER TABLE `case_suspect_assn` ADD FOREIGN KEY case_id_idxfk_1 (`case_id`) REFERENCES `cases` (`id`);

CREATE INDEX `suspect_id_idx` ON `case_suspect_assn`(`suspect_id`);
ALTER TABLE `case_suspect_assn` ADD FOREIGN KEY suspect_id_idxfk (`suspect_id`) REFERENCES `suspect` (`id`);

CREATE INDEX `advocate_id_idx` ON `advocate_case_assn`(`advocate_id`);
ALTER TABLE `advocate_case_assn` ADD FOREIGN KEY advocate_id_idxfk (`advocate_id`) REFERENCES `advocate` (`id`);

CREATE INDEX `case_id_idx` ON `advocate_case_assn`(`case_id`);
ALTER TABLE `advocate_case_assn` ADD FOREIGN KEY case_id_idxfk_2 (`case_id`) REFERENCES `cases` (`id`);
