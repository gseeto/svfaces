<?php
	// This is the HTML template include file (.tpl.php) for personEditPanel.
	// Remember that this is a DRAFT.  It is MEANT to be altered/modified.
	// Be sure to move this out of the drafts/dashboard subdirectory before modifying to ensure that subsequent 
	// code re-generations do not overwrite your changes.
?>
	<div id="formControls">
		<?php $_CONTROL->lblVictimWitnessNum->RenderWithName(); ?>

		<?php $_CONTROL->txtFirstName->RenderWithName(); ?>

		<?php $_CONTROL->txtLastName->RenderWithName(); ?>

		<?php $_CONTROL->calDateOfBirth->RenderWithName(); ?>

		<?php $_CONTROL->txtSocialSecurityNumber->RenderWithName(); ?>

		<?php $_CONTROL->txtPhone->RenderWithName(); ?>

		<?php $_CONTROL->txtEmail->RenderWithName(); ?>

		<?php $_CONTROL->txtNotes->RenderWithName(); ?>

		<?php $_CONTROL->chkGender->RenderWithName(); ?>

		<?php $_CONTROL->lstEthnicity->RenderWithName(); ?>

		<?php $_CONTROL->txtAge->RenderWithName(); ?>

		<?php $_CONTROL->lstPrimaryLanguage->RenderWithName(); ?>

		<?php $_CONTROL->chkDisabled->RenderWithName(); ?>

		<?php $_CONTROL->chkElderly->RenderWithName(); ?>

		<?php $_CONTROL->lstAgencyReferred->RenderWithName(); ?>

		<?php $_CONTROL->lstCasesesAsCase->RenderWithName(true, "Rows=7"); ?>

	</div>

	<div id="formActions">
		<div id="save"><?php $_CONTROL->btnSave->Render(); ?></div>
		<div id="cancel"><?php $_CONTROL->btnCancel->Render(); ?></div>
		<div id="delete"><?php $_CONTROL->btnDelete->Render(); ?></div>
	</div>
