<?php
	 ...
	 * @property Person $_PersonAsPrimary the value for the private _objPersonAsPrimary (Read-Only) if set due to an expansion on the person.primary_language_id reverse relationship
	 * @property Person[] $_PersonAsPrimaryArray the value for the private _objPersonAsPrimaryArray (Read-Only) if set due to an ExpandAsArray on the person.primary_language_id reverse relationship
	 * @property boolean $__Restored whether or not this object was restored from the database (as opposed to created new)
	 */
	class LanguageGen extends QBaseClass {
		 ...
		 * (of type Person), if this Language object was restored with
		...
		protected static function BuildQueryStatement(&$objQueryBuilder, QQCondition $objConditions, $objOptionalClauses, $mixParameterArray, $blnCountOnly) {
			...
			// Create/Build out the QueryBuilder object with Language-specific SELET and FROM fields
			$objQueryBuilder = new QQueryBuilder($objDatabase, 'language');
			Language::GetSelectFields($objQueryBuilder);
			$objQueryBuilder->AddFromItem('language');
		...
		/**
		 * Static Qcodo Query method to query for a single Language object.
		...
		public static function QuerySingle(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			try {
				$strQuery = Language::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, false);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
			...
			// Instantiate a new Language object and return it

			// Do we have to expand anything?
			if ($objQueryBuilder->ExpandAsArrayNodes) {
				$objToReturn = array();
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Language::InstantiateDbRow($objDbRow, null, $objQueryBuilder->ExpandAsArrayNodes, $objToReturn, $objQueryBuilder->ColumnAliasArray);
				...
				return Language::InstantiateDbRow($objDbRow, null, null, null, $objQueryBuilder->ColumnAliasArray);
			}
		}

		/**
		 * Static Qcodo Query method to query for an array of Language objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClauses additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryArray(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			...
			$objDbResult = $objQueryBuilder->Database->Query($strQuery);
			return Language::InstantiateDbResult($objDbResult, $objQueryBuilder->ExpandAsArrayNodes, $objQueryBuilder->ColumnAliasArray);
		}
		...
		/**
		 * Static Qcodo Query method to query for a count of Language objects.
		 * Uses BuildQueryStatment to perform most of the work.
		 * @param QQCondition $objConditions any conditions on the query, itself
		 * @param QQClause[] $objOptionalClauses additional optional QQClause objects for this query
		 * @param mixed[] $mixParameterArray a array of name-value pairs to perform PrepareStatement with
		...
		public static function QueryCount(QQCondition $objConditions, $objOptionalClauses = null, $mixParameterArray = null) {
			// Get the Query Statement
			try {
				$strQuery = Language::BuildQueryStatement($objQueryBuilder, $objConditions, $objOptionalClauses, $mixParameterArray, true);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
...
/*		public static function QueryArrayCached($strConditions, $mixParameterArray = null) {
			...
			$objCache = new QCache('query', 'language_' . serialize($strConditions));
			if (!($strQuery = $objCache->GetData())) {
				// Not Found -- Go ahead and Create/Build out a new QueryBuilder object with Language-specific fields
				$objQueryBuilder = new QQueryBuilder($objDatabase);
				Language::GetSelectFields($objQueryBuilder);
				Language::GetFromFields($objQueryBuilder);
		...
		/**
		 * Updates a QQueryBuilder with the SELECT fields for this Language
		...
		*/
		public static function InstantiateDbRow($objDbRow, $strAliasPrefix = null, $strExpandAsArrayNodes = null, $objPreviousItem = null, $strColumnAliasArray = array()) {
			...
			if (($strExpandAsArrayNodes) && ($objPreviousItem) &&
				...
				$strAlias = $strAliasPrefix . 'personasprimary__victim_witness_num';
				$strAliasName = array_key_exists($strAlias, $strColumnAliasArray) ? $strColumnAliasArray[$strAlias] : $strAlias;
				...
				if ($blnExpandedViaArray)
					return false;
				else if ($strAliasPrefix == 'language__')
					$strAliasPrefix = null;
			}
		...
		public static function InstantiateDbResult(QDatabaseResultBase $objDbResult, $strExpandAsArrayNodes = null, $strColumnAliasArray = null) {
			...
			if ($strExpandAsArrayNodes) {
				$objLastRowItem = null;
				while ($objDbRow = $objDbResult->GetNextRow()) {
					$objItem = Language::InstantiateDbRow($objDbRow, null, $strExpandAsArrayNodes, $objLastRowItem, $strColumnAliasArray);
					if ($objItem) {
						$objToReturn[] = $objItem;
		...
		public static function InstantiateCursor(QDatabaseResultBase $objDbResult) {
			...
			return Language::InstantiateDbRow($objDbRow, null, $strExpandAsArrayNodes, null, $strColumnAliasArray);
		}
		...
		*/
		public static function LoadByName($strName, $objOptionalClauses = null) {
			return Language::QuerySingle(
				QQ::Equal(QQN::Language()->Name, $strName)
			, $objOptionalClauses
			);
		}
		...
		public function Delete() {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Cannot delete this Language with an unset primary key.');
		...
		public function Reload() {
			...
			if (!$this->__blnRestored)
				throw new QCallerException('Cannot call Reload() on a new, unsaved Language object.');

			// Reload the Object
			$objReloaded = Language::Load($this->intId);
		...
		public function Journal($strJournalCommand) {
			$objDatabase = Language::GetDatabase()->JournalingDatabase;
		...
		public static function GetJournalForId($intId) {
			...
			$objResult = $objDatabase->Query('SELECT * FROM language WHERE id = ' .
				$objDatabase->SqlVariable($intId) . ' ORDER BY __sys_date');

			return Language::InstantiateDbResult($objResult);
		}
		...
		public function GetJournal() {
			return Language::GetJournalForId($this->intId);
		}
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case '_PersonAsPrimary':
					...
					// if set due to an expansion on the person.primary_language_id reverse relationship
					// @return Person
				...
				case '_PersonAsPrimaryArray':
					...
					// if set due to an ExpandAsArray on the person.primary_language_id reverse relationship
					// @return Person[]
		...
		*/ 
		public function GetPersonAsPrimaryArray($objOptionalClauses = null) {
			...
			try {
				return Person::LoadArrayByPrimaryLanguageId($this->intId, $objOptionalClauses);
			} catch (QCallerException $objExc) {
				$objExc->IncrementOffset();
				throw $objExc;
			}
		...
		*/ 
		public function CountPeopleAsPrimary() {
			...
			return Person::CountByPrimaryLanguageId($this->intId);
		}
		...
		*/ 
		public function AssociatePersonAsPrimary(Person $objPerson) {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociatePersonAsPrimary on this unsaved Language.');
			if ((is_null($objPerson->VictimWitnessNum)))
				throw new QUndefinedPrimaryKeyException('Unable to call AssociatePersonAsPrimary on this Language with an unsaved Person.');
			...
			$objDatabase->NonQuery('
				UPDATE
					`person`
				SET
					`primary_language_id` = ' . $objDatabase->SqlVariable($this->intId) . '
				WHERE
			...
			if ($objDatabase->JournalingDatabase) {
				$objPerson->PrimaryLanguageId = $this->intId;
		...
		*/ 
		public function UnassociatePersonAsPrimary(Person $objPerson) {
			if ((is_null($this->intId)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociatePersonAsPrimary on this unsaved Language.');
			if ((is_null($objPerson->VictimWitnessNum)))
				throw new QUndefinedPrimaryKeyException('Unable to call UnassociatePersonAsPrimary on this Language with an unsaved Person.');
			...
			// Journaling
			if ($objDatabase->JournalingDatabase) {
				$objPerson->PrimaryLanguageId = null;
				$objPerson->Journal('UPDATE');
			}
		}
		...
		*/ 
		public function UnassociateAllPeopleAsPrimary() {
			...
			// Journaling
			if ($objDatabase->JournalingDatabase) {
				foreach (Person::LoadArrayByPrimaryLanguageId($this->intId) as $objPerson) {
					$objPerson->PrimaryLanguageId = null;
					$objPerson->Journal('UPDATE');
				}
			}
	 ...
	 */
	class QQNodeLanguage extends QQNode {
		...
		public function __get($strName) {
			switch ($strName) {
				...
				case 'PersonAsPrimary':
					return new QQReverseReferenceNodePerson($this, 'personasprimary', 'reverse_reference', 'primary_language_id');
	 ...
	 */
	class QQReverseReferenceNodeLanguage extends QQReverseReferenceNode {
		protected $strTableName = 'language';
		protected $strPrimaryKey = 'id';
		protected $strClassName = 'Language';
		public function __get($strName) {
