<?php
	 ...
	 * listing a collection of Suspect objects.  It includes
	 ...
	 * 
	 */
	class SuspectDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Suspect datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Suspect object, or it can be any QQNode extending from QQN::Suspect().
		 * 
		 ...
		 * sorting using two fields from the Suspect object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Suspect
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Suspect::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Suspect, given the clauses above
			$this->DataSource = Suspect::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Suspect-based QQNode.
		 * It will also verify that it is a proper Suspect-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'suspect') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "suspect".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Suspect()->Id;
				case 'FirstName': return QQN::Suspect()->FirstName;
				case 'LastName': return QQN::Suspect()->LastName;
				case 'DateOfBirth': return QQN::Suspect()->DateOfBirth;
				case 'Pfd': return QQN::Suspect()->Pfd;
				case 'Information': return QQN::Suspect()->Information;
				default: throw new QCallerException('Simple Property not found in SuspectDataGrid content: ' . $mixContent);
