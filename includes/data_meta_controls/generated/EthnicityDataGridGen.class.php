<?php
	 ...
	 * listing a collection of Ethnicity objects.  It includes
	 ...
	 * 
	 */
	class EthnicityDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Ethnicity datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Ethnicity object, or it can be any QQNode extending from QQN::Ethnicity().
		 * 
		 ...
		 * sorting using two fields from the Ethnicity object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Ethnicity
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Ethnicity::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Ethnicity, given the clauses above
			$this->DataSource = Ethnicity::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Ethnicity-based QQNode.
		 * It will also verify that it is a proper Ethnicity-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'ethnicity') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "ethnicity".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Ethnicity()->Id;
				case 'Name': return QQN::Ethnicity()->Name;
				default: throw new QCallerException('Simple Property not found in EthnicityDataGrid content: ' . $mixContent);
