<?php
	 ...
	 * listing a collection of Services objects.  It includes
	 ...
	 * 
	 */
	class ServicesDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Services datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Services object, or it can be any QQNode extending from QQN::Services().
		 * 
		 ...
		 * sorting using two fields from the Services object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Services
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Services::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Services, given the clauses above
			$this->DataSource = Services::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Services-based QQNode.
		 * It will also verify that it is a proper Services-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'services') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "services".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Services()->Id;
				case 'ServicesTypeId': return QQN::Services()->ServicesTypeId;
				case 'PersonId': return QQN::Services()->PersonId;
				case 'Person': return QQN::Services()->Person;
				default: throw new QCallerException('Simple Property not found in ServicesDataGrid content: ' . $mixContent);
