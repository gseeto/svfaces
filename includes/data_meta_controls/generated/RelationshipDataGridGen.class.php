<?php
	 ...
	 * listing a collection of Relationship objects.  It includes
	 ...
	 * 
	 */
	class RelationshipDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Relationship datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Relationship object, or it can be any QQNode extending from QQN::Relationship().
		 * 
		 ...
		 * sorting using two fields from the Relationship object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Relationship
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Relationship::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Relationship, given the clauses above
			$this->DataSource = Relationship::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Relationship-based QQNode.
		 * It will also verify that it is a proper Relationship-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'relationship') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "relationship".');
				...
				default: throw new QCallerException('Simple Property not found in RelationshipDataGrid content: ' . $mixContent);
