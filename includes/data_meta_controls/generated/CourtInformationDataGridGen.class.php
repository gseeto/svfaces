<?php
	 ...
	 * listing a collection of CourtInformation objects.  It includes
	 ...
	 * 
	 */
	class CourtInformationDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CourtInformation datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CourtInformation object, or it can be any QQNode extending from QQN::CourtInformation().
		 * 
		 ...
		 * sorting using two fields from the CourtInformation object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CourtInformation
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CourtInformation::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CourtInformation, given the clauses above
			$this->DataSource = CourtInformation::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CourtInformation-based QQNode.
		 * It will also verify that it is a proper CourtInformation-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'court_information') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "court_information".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CourtInformation()->Id;
				case 'Information': return QQN::CourtInformation()->Information;
				default: throw new QCallerException('Simple Property not found in CourtInformationDataGrid content: ' . $mixContent);
