<?php
	 ...
	 * listing a collection of CaseStatus objects.  It includes
	 ...
	 * 
	 */
	class CaseStatusDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this CaseStatus datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the CaseStatus object, or it can be any QQNode extending from QQN::CaseStatus().
		 * 
		 ...
		 * sorting using two fields from the CaseStatus object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from CaseStatus
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = CaseStatus::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from CaseStatus, given the clauses above
			$this->DataSource = CaseStatus::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a CaseStatus-based QQNode.
		 * It will also verify that it is a proper CaseStatus-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'case_status') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "case_status".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::CaseStatus()->Id;
				case 'Name': return QQN::CaseStatus()->Name;
				default: throw new QCallerException('Simple Property not found in CaseStatusDataGrid content: ' . $mixContent);
