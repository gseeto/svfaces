<?php
	 ...
	 * listing a collection of Language objects.  It includes
	 ...
	 * 
	 */
	class LanguageDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Language datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Language object, or it can be any QQNode extending from QQN::Language().
		 * 
		 ...
		 * sorting using two fields from the Language object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Language
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Language::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Language, given the clauses above
			$this->DataSource = Language::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Language-based QQNode.
		 * It will also verify that it is a proper Language-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'language') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "language".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Language()->Id;
				case 'Name': return QQN::Language()->Name;
				default: throw new QCallerException('Simple Property not found in LanguageDataGrid content: ' . $mixContent);
