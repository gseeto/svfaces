<?php
	 ...
	 * listing a collection of Grant objects.  It includes
	 ...
	 * 
	 */
	class GrantDataGridGen extends QDataGrid {
		/**
		 ...
		 * way of adding a column to this Grant datagrid.  The description of a column's
		 * content can be either a text string description of a simple field name
		 * in the Grant object, or it can be any QQNode extending from QQN::Grant().
		 * 
		 ...
		 * sorting using two fields from the Grant object.
		 *
		 ...
		 *
		 * @param mixed $mixContent string or QQNode from Grant
		...
		public function MetaDataBinder(QQCondition $objCondition = null, $objOptionalClauses = null) {
			...
			if ($this->Paginator) $this->TotalItemCount = Grant::QueryCount($objCondition, $objClauses);
			...
			// Set the DataSource to be a Query result from Grant, given the clauses above
			$this->DataSource = Grant::QueryArray($objCondition, $objClauses);
		}
		 ...
		 *
		 * Given a QQNode or a Text String, this will return a Grant-based QQNode.
		 * It will also verify that it is a proper Grant-based QQNode, and will throw an exception otherwise.
		 *
		...
		protected function ResolveContentItem($mixContent) {
			if ($mixContent instanceof QQNode) {
				...
				if ($mixContent->_RootTableName == 'grant') {
					...
					throw new QCallerException('Content QQNode has a root table of "' . $mixContent->_RootTableName . '". Must be a root of "grant".');
			} else if (is_string($mixContent)) switch ($mixContent) {
				case 'Id': return QQN::Grant()->Id;
				case 'Name': return QQN::Grant()->Name;
				default: throw new QCallerException('Simple Property not found in GrantDataGrid content: ' . $mixContent);
